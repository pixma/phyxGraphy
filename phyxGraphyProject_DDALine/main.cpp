...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow){
    ...
    gMainWindowHandle = CreateWindow(TEXT("phyxgraphywindow"), TEXT("phyxGraphy : Line drawing: DDA Alogorithm"), WS_VISIBLE|WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 750, 550, NULL, NULL, hInstance, NULL);
...
void __stdcall InitializeWindowComponents(HWND hWindow){
  ...
  //   ghConsoleTextArea = CreateWindow("EDIT","", WS_CHILD | WS_VISIBLE | ES_LEFT | ES_AUTOVSCROLL | ES_MULTILINE | ES_READONLY, 5, (rcClient.bottom - rcClient.top)/2 - 20, (rcClient.right - rcClient.left) - 10,(rcClient.bottom - rcClient.top)/2 -10, hWindow, (HMENU) EDITCONSOLE, gInstance, NULL);
     ...
     gGroupBox = CreateWindow("button","Controls: Line Drawing", WS_CHILD| WS_VISIBLE| BS_GROUPBOX, 2, 2, rcClient.right - rcClient.left - 5, rcClient.bottom - rcClient.top - 5, gRightSplitterWindow, (HMENU)103, gInstance, NULL);
     ...
     gGroupBoxCoordinateB = CreateWindow("button","Second Coordinate", WS_CHILD| WS_VISIBLE| BS_GROUPBOX , 10, (rcClient.bottom - rcClient.top)/2 , rcClient.right - rcClient.left - 25, (rcClient.bottom - rcClient.top)/2 - 15, gGroupBox, (HMENU)105, gInstance, NULL);
     ...
     gUDControlAX = CreateUpDownControl(WS_VISIBLE| WS_CHILD| WS_BORDER| UDS_ALIGNRIGHT|UDS_AUTOBUDDY|UDS_SETBUDDYINT| UDS_ARROWKEYS, (rcClient.right - rcClient.left)/2+165, 25, 20, 20, gRightSplitterWindow, UPDOWNCONTROLCOORDINATEAX, gInstance, gBuddyEditAX, 50, -50, 40);
     ...
     gUDControlAY = CreateUpDownControl(WS_VISIBLE| WS_CHILD| WS_BORDER| UDS_ALIGNRIGHT|UDS_AUTOBUDDY|UDS_SETBUDDYINT| UDS_ARROWKEYS, (rcClient.right - rcClient.left)/2+75, 65, 20, 20, gRightSplitterWindow, UPDOWNCONTROLCOORDINATEAY, gInstance, gBuddyEditAY, 50, -50, 20);
     ...
     hWndStaticC = CreateWindow("STATIC", "Value of Coordinate x:", WS_CHILD|WS_VISIBLE|SS_LEFT,50,  (rcClient.bottom - rcClient.top)/2 +28, 200, 20, gRightSplitterWindow, (HMENU) 110, gInstance, NULL);
     hWndStaticD = CreateWindow("STATIC", "Value of Coordinate y:", WS_CHILD|WS_VISIBLE|SS_LEFT,50,  (rcClient.bottom - rcClient.top)/2 +68, 200, 20, gRightSplitterWindow, (HMENU) 111, gInstance, NULL);
     
     InitOpenGLEnvironment(gLeftHdc);
     RenderScene();
     
//     ddaLineDraw(0.40, 0.3, 0.8, 0.6);
...
void __stdcall ddaLineDraw(int x0, int y0, int x1, int y1){
...
//     glPushMatrix();     
     ...
     float xIncermental, yIncermental, x = x0, y = y0;
     int steps, k;
     ...
     xIncermental = (float)dx / (float)steps;
     yIncermental = (float)dy / (float)steps;
     glColor3f (1.0f, 1.0f, 1.0f);
     glBegin(GL_LINES);
     glVertex2i(x, y);
     insertAtListBox((int)x, (int)y);
     glEnd();
...
void __stdcall insertAtListBox(int x, int y){
